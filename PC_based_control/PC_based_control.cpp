...
2. .NET Framework Basic
	...
	4) Code : 아이콘 오른쪽 마우스 - 코드 수정
		...
		4] 배열
			[1] string 배열 생성 : string[] 변수명 = new string[원소개수] { "원소1", "원소2", ... };
			[1] bool 배열 생성 : bool[] 변수명 = new bool[원소개수] { bool형, bool형, ... };
	...
	5) 무한루프
		...
		(2) for
			for( ; ; )
				코드
	6) break
	7) continue : 반복문을 깨지 않고, 해당 반복문의 다음 반복 실행

5. 점프문
	1) goto : 지정된 레이블로 실행 이동
		- 스파게티 코드 가능성 증가
		- 이득을 볼 수 있는 경우가 있다
